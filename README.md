...
5. If a pre-release is not given then as `ContinousDelivery` indicates, we once <ins>increment the patch by one</ins>. If the patch was `0`, then it won't exceed `1` in this calculation. Why patch? Because a change in version core signalizes a *new release*. Why only patch? First of all, there are different [versioning modes](./docs/configuration-file.md#versioning-mode) but `ContinousDelivery` is one of the more feasible workflows that are easy introducable, so if you want for example introduce a minor or major release, you would have to adjust the versioning preset to apply <sub>your convention</sub> that affects not only patch anymore or you create a tag that reflects the next minor or major release and [vernuntii will just consider it](#version-adaptivity) in next version calculation.
