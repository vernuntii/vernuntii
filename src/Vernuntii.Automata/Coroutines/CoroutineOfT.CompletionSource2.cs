...
partial struct Coroutine<T>
{
    ...
    internal abstract class AbstractCompletionSource2 : IValueTaskSource<T>, IValueTaskSource
    {
        ...
        public ExceptionDispatchInfo? ExceptionalResult;
        ...
        {
            ExceptionalResult = ExceptionDispatchInfo.Capture(error);
    ...
    private sealed class SyncSuccessSentinelStateMachineBox2 : AbstractCompletionSource
    {
        public SyncSuccessSentinelStateMachineBox2() => SetResult(default!);
    }

    /// <summary>Provides a strongly-typed box object based on the specific state machine type in use.</summary>
    internal sealed class CompletionSource2 : AbstractCompletionSource2, IValueTaskSource<T>, IValueTaskSource
    {
        ...
        [ThreadStatic]
        private static CompletionSource2? t_tlsCache;
        ...
        internal static CompletionSource2 RentFromCache()
        {
            // First try to get a box from the per-thread cache.
            CompletionSource2? box = t_tlsCache;
            if (box is not null) {
                t_tlsCache = null;
            } else {
                // If we can't, then try to get a box from the per-core cache.
                ref CompletionSource2? slot = ref PerCoreCacheSlot;
                if (slot is null ||
                    (box = Interlocked.Exchange<CompletionSource2?>(ref slot, null)) is null) {
                    // If we can't, just create a new one.
                    box = new CompletionSource2();
                }
            }
        ...
        private static ref CompletionSource2? PerCoreCacheSlot {
            ...
            get {
                ...
                Debug.Assert(transientValue is null || transientValue is CompletionSource2,
                    $"Expected null or {nameof(CompletionSource2)}, got '{transientValue}'");
#endif
                return ref Unsafe.As<object?, CompletionSource2?>(ref s_perCoreCache[i].Object);
            }
        }
        ...
        private static void ExecutionContextCallback(object? s)
        {
            ...
            Debug.Assert(s is CompletionSource2, $"Expected {nameof(CompletionSource2)}, got '{s}'");
            //Unsafe.As<CompletionSource2>(s).StateMachine!.MoveNext();
        }
