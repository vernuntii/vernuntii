...
static (string FileName, string FileContent) GenerateEffectCallExtensions(int from, int to)
{
    ...
    for (var currentTo = from; currentTo <= to; currentTo++)
    {
        ...
        sb.AppendLine($$"""
                public static Coroutine<Coroutine> Launch<{{genericParameters}}>(Func<{{genericParameters}}, Coroutine> provider, {{parameters}}) => LaunchInternal(provider, new Closure<{{genericParameters}}>({{arguments}}));

                public static Coroutine<Coroutine<TResult>> Launch<{{genericParameters}},TResult>(Func<{{genericParameters}},Coroutine<TResult>> provider, {{parameters}}) => LaunchInternal<TResult>(provider, new Closure<{{genericParameters}}>({{arguments}}));
            """);
        if (currentTo < to)
        {
            sb.AppendLine();
