...
{
    internal static partial class libgit2
    {
        ...
        /// this one) that have not subsequently been _shutdownFinalizer.
        /// </para>
        /// </summary>
        ...
        /// have not been _shutdownFinalizer (after this one).
        /// </para>
        /// </summary>
        ...
        /// <returns>1 if the HEAD is detached, 0 if it's not, or an error code</returns>
        [DllImport(LibGit2Binary, CallingConvention = CallingConvention.Cdecl)]
        public static extern unsafe int git_repository_is_shallow(
            git_repository* repo);
        ...
        /// <summary>
        /// By using <see cref="CriticalFinalizerObject"/> you can be sure that the finalizer is called in critical situations.
        /// </summary>
        private sealed class Lifetime : IDisposable
        {
            static string GetRuntimeIdentifier()
            {
                if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux)) {
                    ...
                    throw new NotSupportedException($"A pre-compiled version of libgit2 does not exist for the current operating system: {RuntimeInformation.OSDescription}");
                }
            }
            ...
            private sealed class ShutdownFinalizer : CriticalFinalizerObject, IDisposable
            {
                ...
                /// <summary>
                /// Calls <see cref="git_libgit2_shutdown"/>.
                /// </summary>
                public void Dispose()
                {
                    var initCounter = _initCounter;

                    if (Interlocked.CompareExchange(ref _initCounter, 0, initCounter) != 0) {
                        return;
                    }
